...
def saveInDirectory():
    ...
    try:
        ...
        if not OSError:
            print(f"A new folder named {folder_name} was successfully created!")
    ...
    while os.path.exists(f'./{folder_name}/{filename}.txt') == True:
        print(f'{filename}.txt already exists!')
        filename = input("Please choose another filename: ")
        if os.path.exists(f'./{folder_name}/{filename}.txt') == False:
            break
    ...
    with open(f'./{folder_name}/{filename}.txt', 'w') as file:
        ...
        file.close()
        print(f'{filename}.txt was successfully created in folder {folder_name}!')
